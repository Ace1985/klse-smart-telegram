...
STOCK_NAMES = {
    '1155': 'MAYBANK', '1023': 'CIMB', '1295': 'PBBANK', '1146': 'RHBBANK',
    '4162': 'HLBANK', '4707': 'AMBANK', '5347': 'TENAGA', '6742': 'YTLPOWR',
    '4677': 'YTL', '5211': 'SUNWAY', '5014': 'GAMUDA', '6947': 'CelcomDigi',
    '6888': 'AXIATA', '5225': 'IHH', '5183': 'PCHEM', '0166': 'INARI',
    '0138': 'MYEG', '0097': 'VITROX', '0208': 'GREATEC', '0273': 'NATGATE',
    '0256': 'UMC', '5341': 'LACMED'
}

POSITIVE_NEWS = [
    'profit growth', 'profit rises', 'profit surge', 'earnings growth',
    'earnings beat', 'revenue growth', 'contract win', 'new contract',
    'dividend', 'upgrade', 'surge', 'growth', 'strong demand'
]

NEGATIVE_NEWS = [
    'profit drop', 'profit falls', 'profit decline', 'loss',
    'downgrade', 'risk', 'lawsuit', 'weak', 'drop',
    'decline', 'guidance cut', 'slowdown', 'warning'
]
...
def analyze(symbol, m):
    ...
    if de is not None:
        de /= 100
    if rg is None:
        rg = growth_from(income, ['Total Revenue', 'Operating Revenue'])
    if pg is None:
        pg = growth_from(income, ['Net Income', 'Net Income Common Stockholders'])
    ...
    name = f'{name} ({code})' if name != code else code
    intra = intraday(symbol)

    ap = 0
    if intra.get('change') is not None:
        ap += 2 if intra['change'] > 1 else -2 if intra['change'] < -1 else 0
    if intra.get('vwap_diff') is not None:
        ap += 2 if intra['vwap_diff'] > .5 else -2 if intra['vwap_diff'] < -.5 else 0
    if price is not None and ma50 is not None:
        ap += 1 if price > ma50 else -1
    ...
    if intra.get('volume_ratio') is not None and intra['volume_ratio'] >= 1.5:
        ap += 1 if (intra.get('change') or 0) >= 0 else -1
    if m.get('chg') is not None:
        ap += 1 if m['chg'] > .3 else -1 if m['chg'] < -.3 else 0
    ...
    if rsi14 is not None:
        np += 1 if 45 <= rsi14 <= 65 else -2 if rsi14 > 75 else 1 if rsi14 < 30 else 0
    if intra.get('volume_ratio') is not None and intra['volume_ratio'] >= 1.5:
        np += 1 if (intra.get('change') or 0) >= 0 else -1
    np += news_score
    if m.get('chg') is not None:
        np += 1 if m['chg'] > .3 else -1 if m['chg'] < -.3 else 0
    ...
    return dict(
        ...
        intra=intra, afternoon=afternoon, nextday=nextday, ap=ap, np=np
    )
...
def trend_text(x):
    ...
    if None not in (p, a, b, c):
        return '🟢 Strong Bullish' if p > a > b > c else '🟢 Bullish' if p > b else '🟡 Pullback' if p > c else '🔴 Bearish'
...
def build(x):
    ...
    vtxt = 'N/A' if vol is None else f'{vol:.2f}x'
    text = (
...
def send(text):
    if not TOKEN or not CHAT_ID:
        raise RuntimeError('Missing TELEGRAM_TOKEN/TELEGRAM_CHAT_ID')
...
def main():
    ...
    if mode == 'lunch':
        ...
        forecast = '🟢 Afternoon bias stronger' if bp > bw else '🔴 Afternoon bias weaker' if bw > bp else '🟡 Afternoon mixed'
    else:
        bp = sum(x['np'] >= 2 for x in out)
        bw = sum(x['np'] <= -2 for x in out)
        forecast = '🟢 Next-day bias stronger' if bp > bw else '🔴 Next-day bias weaker' if bw > bp else '🟡 Next-day mixed'
    
    lines = [
        f'<b>{title}</b>',
        f"{datetime.now(TZ):%d %b %Y %H:%M}",
        '',
        macro_text(m),
    ...
    send('\n'.join(lines))
    print(f'{mode} report sent: {len(out)} stocks')
